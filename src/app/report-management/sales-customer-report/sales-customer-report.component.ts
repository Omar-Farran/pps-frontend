...
@Component({
  ...
  styleUrls: ['./sales-customer-report.component.scss']
})
export class SalesCustomerReportComponent {
columns: any[] = [
    { name: "sales-customer-report.customer", field: "customerName" },
...
branchies:any;
  ...
  private getList () 
  {
    this.baseService.Post('Report', 'GetSalesByCustomerReport', this.baseSearch).subscribe
    ( res => 
      {
...
downloadReport() {
  this.baseService.generatePDF('Report', 'GetSalesByCustomerReportePDF', this.baseSearch)
    .subscribe((blob: Blob) => {
      debugger;
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'Sales Customer Report.pdf'; // 👈 change the filename as needed
      a.click();
