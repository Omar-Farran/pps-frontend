...
@Component({
  ...
  styleUrls: ['./sales-period-report.component.scss']
})
export class SalesPeriodReportComponent {
columns: any[] = [
    { name: "sales-period-report.period", field: "period" },
    { name: "sales-period-report.invoicesCount", field: "invoicesCount" , type:'number' },
    { name: "sales-period-report.totalAmount", field: "totalAmount"  , type:'number'},
...
branchies:any;
  ...
  private getList () 
  {
    this.baseService.Post('Report', 'GetSalesPeriodReport', this.baseSearch).subscribe
    ( res => 
      {
...
downloadReport() {
  this.baseService.generatePDF('Report', 'GenerateSalesPeriodInvoicePDF', this.baseSearch)
    .subscribe((blob: Blob) => {
      debugger;
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'Sales Period Report.pdf'; // 👈 change the filename as needed
      a.click();
