...
export class PurchaseStatusReportComponent {
columns: any[] = [
    { name: "purchase-status-report.invoiceNumber", field: "invoiceNumber" },
    { name: "purchase-status-report.invoiceDate", field: "invoiceDate" , type:'date' },
    { name: "purchase-status-report.supplier", field: "customer" },
    { name: "purchase-status-report.createdUser", field: "createdUser" },
    { name: "purchase-status-report.status", field: "statusName" ,  isTranslate:true },
    { name: "purchase-status-report.totalAmount", field: "totalAmount" , type:'number' }
  ];
...
branchies:any;
  ...
  private getList () 
  {
    this.baseService.Post('Report', 'GetPurchaseInvoiceList', this.baseSearch).subscribe
    ( res => 
      {
...
downloadReport() {
  this.baseService.generatePDF('Report', 'GeneratePurchaseInvoicePDF', this.baseSearch)
    .subscribe((blob: Blob) => {
      debugger;
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'Purchase Status Report.pdf'; // 👈 change the filename as needed
      a.click();
